...
public class Unit
{
    ...
    public List<ConstEffectInfo> GetActualConstEffects()
    {
      ...
      foreach (var constEffect in ConstEffects)
      {
        ...
        else if (constEffectInfo.TimestampEnd.HasValue && c.TimestampEnd.HasValue && constEffectInfo.TimestampEnd.Value < c.TimestampEnd.Value)
        {
    ...
    public CardAbility? AbilityCard => AbilityKey.HasValue ? Databases.Catalogue.GetCard<CardAbility>(AbilityKey.Value) : null;
